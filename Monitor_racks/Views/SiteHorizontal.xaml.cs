...
public partial class SiteHorizontal : ContentPage
{
    ...
    private async void ActualizarVista()
    {
        ...
        if (fWidth > fHeight)
        {
            gSite?.Add(ActualizarALandscapteLayout(), 0, 0);
        }
        else
        {
            await Shell.Current.GoToAsync($"Principal", false);
            await Shell.Current.GoToAsync($"Site?ID={ ID }");
        }
        Content = default;
        Content = gSite;
    }
    ...
    private Grid ActualizarALandscapteLayout()
    {
        ...
        gRacks.Add(gRack2, 1, 0);
        gRacks.Add(gRack3, 0, 1);
        gRacks.Add(gRack4, 1, 1);
